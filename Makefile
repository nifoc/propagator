...
upload-docs: docs
	$(gen_verbose) rsync -avz --no-o --no-g -e ssh --chmod=og=r -p --delete --exclude '*.edoc' --exclude 'edoc-info' doc/ kempkens.io:/var/www/nifoc/$(PROJECT)/$(PROJECT_VERSION)
	@ssh kempkens.io chown -R www-data:www-data /var/www/nifoc/$(PROJECT)/$(PROJECT_VERSION)
